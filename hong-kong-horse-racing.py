...
os.chdir("C:\\Users\\u5326\\.spyder-py3\\horse_race\\race-master\\hong-kong-horse-racing")
...
#test,train分割
label = data_new['finishing_position'].astype(np.float32)
label = np.array(label)-1
dataset = data_new.drop(['finishing_position'], axis=1).astype(np.float32)
...
#linregress
import horse_racing_linregress as hrl
X = np.array(data_new['win_odds'].astype(np.float16))
Y = np.array(data_new['finishing_position'].astype(np.int8))
hrl.linregress(X, Y)
